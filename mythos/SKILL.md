...
name: mythos
description: >-
  Opus 4.8을 "미토스급 시스템"으로 끌어올리는 멀티에이전트 코딩 하네스 (기본판, 외부 모델 불필요). 어려운 구현·대형
  리팩터·마이그레이션처럼 정답을 테스트로 정의할 수 있는 코딩 태스크를 plan → 병렬 변형 구현(테스트 자기수정 T1 루프)
  → 적대적 검증(Claude 자체) → 합성 파이프라인으로 처리해, 단발 Opus보다 정확도·장기 자율성을 끌어올린다.
  Claude Code의 Workflow 오케스트레이션만으로 동작한다. Use when 사용자가 "미토스", "미토스급으로",
  "여러 변형 만들어 테스트로 거르고 적대검증", "하드한 구현 빡세게", 또는 어려운 코딩/에이전트 작업을 멀티에이전트로
  풀고 싶다고 할 때. 교차모델(GPT-5.5) 적대 검증이 필요하면 mythos-x 스킬을 쓸 것. 쉬운 단발 질문·간단 수정엔 쓰지 말 것(비용 큼).
---

# Mythos-class harness (기본판)

Opus 4.8을 미토스급 **시스템**으로 감싸는 멀티에이전트 하네스. 모델 가중치를 바꾸는 게 아니라, `plan → 병렬 변형 → 테스트 자기수정 → 적대 검증 → 합성`으로 **실효 성능**(정확도·장기 자율성)을 끌어올린다. 효과가 큰 건 하네스로 닫히는 갭(장기 자율/코딩 정확도)이고, 단발 추론 한계 자체를 바꾸진 않는다.

**이 기본판은 외부 모델 없이 Claude Code만으로 동작한다.** 적대 검증을 GPT-5.5(Codex)에게 맡기는 강화판은 `mythos-x` 스킬을 쓸 것.
...
3. **Workflow 실행** — 이 SKILL.md와 **같은 디렉토리의 `mythos-class.js`를 Read**한 뒤, 그 내용을 Workflow의 `script` 인자로 인라인 전달한다:
   ```
   Workflow({
